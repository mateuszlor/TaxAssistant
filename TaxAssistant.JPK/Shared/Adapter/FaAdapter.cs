...
namespace TaxAssistant.JPK.Shared.Adapter
{
    ...
    {
        public Fa? Adapt(JPK_FA item)
		{
			if (item == null
				|| item.Podmiot1 == null
				|| item.Podmiot1.IdentyfikatorPodmiotu == null
				|| item.FakturaWierszCtrl == null
				|| item.FakturaCtrl == null
				|| item.Naglowek == null)
			{
				return null;
			}
			...
			return new Fa
			{
				Subject = Adapt(item.Podmiot1),
				ControlData = AdaptControlData(item.FakturaWierszCtrl, item.FakturaCtrl, item.ZamowienieCtrl),
				Header = Adapt(item.Naglowek),
				Orders = item.Zamowienie?.Select(Adapt).ToList() ?? [],
				Invoices = item.Faktura?.Select(x => Adapt(x, item.FakturaWiersz)).ToList() ?? [],
			};
		}

        private static FaCompany Adapt(JPKPodmiot1 item) => new FaCompany
        {
            Name = item.IdentyfikatorPodmiotu.PelnaNazwa,
            TaxIdentificationNumber = item.IdentyfikatorPodmiotu.Item,
            TaxIdentificationNumberType = Enum.Parse<TaxIdentificationNumberType>(item.IdentyfikatorPodmiotu.ItemElementName.ToString()),
            Address = AdaptAddress(item.Item)
        };

        private static FaHeader Adapt(JPKNaglowek item) => new FaHeader
        {
            DateFrom = item.DataOd,
            DateTo = item.DataDo,
            GenerationDate = item.DataWytworzeniaJPK,
            FormCode = item.KodFormularza?.Value.ToString(),
            FormVariant = item.WariantFormularza,
            Purpose = (JpkPurpose)item.CelZlozenia,
            TaxOfficeCode = item.KodUrzedu.ToString(),
        };
        ...
        private static FaControlData AdaptControlData(JPKFakturaWierszCtrl? invoiceRowControl, JPKFakturaCtrl? invoiceControl, JPKZamowienieCtrl? orderControl) => new FaControlData
        {
            InvoiceRowsCount = ParseNullableIntString(invoiceRowControl?.LiczbaWierszyFaktur),
            TotalIncomeFromRows = invoiceRowControl?.WartoscWierszyFaktur ?? 0,
            InvoicesCount = ParseNullableIntString(invoiceControl?.LiczbaFaktur),
            TotalIncomeFromInvoices = invoiceControl?.WartoscFaktur ?? 0,
            OrdersCount = ParseNullableIntString(orderControl?.LiczbaZamowien),
            TotalIncomeFromOrders = orderControl?.WartoscZamowien ?? 0
        };
        ...
        private static FaCompanyAddress? AdaptAddress(object item) => item switch
        {
        ...
        private static int ParseNullableIntString(string? item) => item == null
            ? 0
            : int.Parse(item);
    }
}
