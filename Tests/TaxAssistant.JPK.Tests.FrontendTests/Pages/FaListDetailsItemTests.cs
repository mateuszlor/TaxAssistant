...
public class FaListDetailsItemTests : BunitTestContext
{
    ...
    public void FaListDetailsItem_ForFoundInvoiceWithRows_ShouldDisplay()
    {
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            rows[i].ChildNodes[3].FirstChild?.TextContent.Trim().Should().Be(invoiceRow.UnitPriceNet.ToString("N", CultureInfo.CurrentCulture));
            rows[i].ChildNodes[4].FirstChild?.TextContent.Trim().Should().Be(invoiceRow.TotalPriceNet.ToString("N", CultureInfo.CurrentCulture));
            rows[i].ChildNodes[5].FirstChild?.TextContent.Trim().Should().Be(invoiceRow.TotalPriceGross!.Value.ToString("N", CultureInfo.CurrentCulture));
        }
    }
}
